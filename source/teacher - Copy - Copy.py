...
class Teacher(nn.Module):
    ...
    def computeLoss(self, input_ids_1, labels_1, input_ids_2, labels_2):
        with torch.no_grad():
            outputs_1 = self.forward(input_ids=input_ids_1, output_hidden_states=True)
            outputs_2 = self.forward(input_ids=input_ids_2, output_hidden_states=True)
        loss_fct = CrossEntropyLoss()

        half_layers = self.num_layers // 2 #Half allocation, we split the layers here
        outputs_1 = outputs_1.hidden_states[:half_layers+1]
        outputs_2 = outputs_2.hidden_states[half_layers+1:]

        #grab the last layer
        last_layer_1 = outputs_1[-1]
        last_layer_2 = outputs_2[-1]

        #Calculate the logits
        logits_1 = self.base_model.lm_head(last_layer_1)
        logits_2 = self.base_model.lm_head(last_layer_2)
