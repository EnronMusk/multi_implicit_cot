...
class Teacher(nn.Module):
    def __init__(self, config):
        ...
        #Create custom embeddings for each problem.
        self.embedding_1 = nn.Embedding(self.base_model.config.vocab_size, hidden_size//2)
        self.embedding_2 = nn.Embedding(self.base_model.config.vocab_size, hidden_size//2)

        #Create custom lm_heads for each problem.
        self.lm_head_1 = nn.Linear(hidden_size//2, self.base_model.config.vocab_size)
        self.lm_head_2 = nn.Linear(hidden_size//2, self.base_model.config.vocab_size)

    def forward(self, input_ids_1, input_ids_2, output_hidden_states=False):
        #Embed both problems individually.
        embedding_1 = self.embedding_1(input_ids_1)
        embedding_2 = self.embedding_2(input_ids_2)

        #print(input_ids_1[0])
        #print(input_ids_2[0])

        #print(self.tokenizer.decode(input_ids_1[0], skip_special_tokens=True))
        #print(self.tokenizer.decode(input_ids_2[0], skip_special_tokens=True))

        #print(len(embedding_1))
        #print(len(embedding_1[0]))
        #print(len(embedding_1[0][0]))

        
        #print(len(embedding_2))
        #print(len(embedding_2[0]))
        #print(len(embedding_2[0][0]))

        input_embeddings_both = torch.cat([embedding_1, embedding_2], dim=-1)

        #print(len(input_embeddings_both))
        #print(len(input_embeddings_both[0]))
        #print(len(input_embeddings_both[0][0]))

        outputs = self.base_model.forward(inputs_embeds=input_embeddings_both, output_hidden_states=output_hidden_states)
        return outputs
    ...
    def computeLoss(self, input_ids_1, labels_1, input_ids_2, labels_2):
        with torch.no_grad():
            outputs = self.forward(input_ids_1=input_ids_1, input_ids_2=input_ids_2, output_hidden_states=True)
        loss_fct = CrossEntropyLoss()

        layer_size = self.hidden_size
        #print(outputs.hidden_states[12][0][0][0])
        ...
        #print(outputs[0][0][0])
        outputs = outputs.hidden_states[self.num_layers]


        #grab the last layer
        outputs_1, outputs_2 = outputs.split(layer_size//2, dim=-1)

        #print(last_layer_1)

        #Calculate the logits
        logits_1 = self.lm_head_1(outputs_1)
        logits_2 = self.lm_head_2(outputs_2)

        labels_pred_1 = logits_1.argmax(-1)
        mask = labels_1[...,1:].ge(0)
        correct_tokens_1  = ((labels_pred_1[...,:-1] == labels_1[...,1:]) * mask).sum()
        total_tokens_1  = mask.sum()
        token_accuracy_1 = correct_tokens_1  / total_tokens_1 

        shift_logits = logits_1[..., :-1, :].contiguous()
        shift_labels = labels_1[..., 1:].contiguous()
        loss_1 = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

        labels_pred_2 = logits_2.argmax(-1)
        mask = labels_2[...,1:].ge(0)
        correct_tokens_2  = ((labels_pred_2[...,:-1] == labels_2[...,1:]) * mask).sum()
        total_tokens_2  = mask.sum()
        token_accuracy_2 = correct_tokens_2  / total_tokens_2 

        shift_logits = logits_2[..., :-1, :].contiguous()
        shift_labels = labels_2[..., 1:].contiguous()
        loss_2 = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

        token_accuracy = (correct_tokens_1 + correct_tokens_2) / (total_tokens_1 + total_tokens_2)
        total_correct_tokens = correct_tokens_1 + correct_tokens_2
        total_tokens = total_tokens_1 + total_tokens_2
        total_loss = loss_1 + loss_2


        #print(labels_pred_1[...,:-1][0])
        #print(labels_1[...,1:][0])

        #print(self.tokenizer.decode(labels_pred_1[...,:-1][0], skip_special_tokens=True))
        #print(self.tokenizer.decode(labels_1[...,1:][0], skip_special_tokens=True))
        #print(self.tokenizer.decode(labels_pred_2[...,:-1][0], skip_special_tokens=True))
        #print(self.tokenizer.decode(labels_2[...,1:][0], skip_special_tokens=True))

        outputs = {
            "loss_1": loss_1,
            "token_accuracy_1": token_accuracy_1,
            "loss_2": loss_2,
            "token_accuracy_2": token_accuracy_2,
    ...
    def evaluate(self, dataloader : DataLoader, ctx):
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            with ctx:
                outputs = self.computeLoss(input_ids_1, labels_1, input_ids_2, labels_2)
            ...
            for i, (input_ids_all_i, beam_output_i) in enumerate(zip(input_ids_2, beam_output)):
                sub_iteration += 1
    ...
    def trainModel(self, train_handler : DatasetHandler, test_handler : DatasetHandler, limit : float) -> None:
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                outputs = self.computeLoss(input_ids_1, labels_1, input_ids_2, labels_2)
            loss = 0.5*outputs['loss_1'] + 0.5*outputs['loss_2']
            token_accuracy = outputs['token_accuracy'].item()
            token_accuracy_1 = outputs['token_accuracy_1'].item()
            token_accuracy_2 = outputs['token_accuracy_2'].item()
            ...
            if iteration % math.floor(len(train_dataloader)/10+1) == 0:
                print (f"Step: {iteration}. PPL: {ppl:.6f}. Training Accuracy: {token_accuracy:.6f}; P1 Train {token_accuracy_1:.6f}; P2 Train {token_accuracy_2:.6f}")
            iteration += 1
