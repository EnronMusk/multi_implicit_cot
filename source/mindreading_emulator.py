...
from source.configurations import StudentConfig
...
class StudentMindReader(nn.Module):
    def __init__(self, config : StudentConfig, teacher : Teacher):
        super().__init__()
        self.config = config
    ...
    def train(self, train_handler : DatasetHandler, test_handler : DatasetHandler, limit : float) -> None:
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            if quasi_train_accuracy > limit:
                print(f"Quasi accuracy limit reached, stopping quasi training at quasi training accuracy: {quasi_train_accuracy:.6f}.")
                break
        ...
        emulator.save_pretrained(os.path.join(train_handler.path+r'\models\emulator'))
